      ...
      <div class="jumbo-content-container">
        <div class="container mx-auto">
          <section class="flex flex--column flex--items-center">
            <img src="/assets/img/gov-logo.png" alt="Imo State Government" class="state-logo" />
            <h1
              class="main-heading text--center text--white"
            >IMO STATE HEALTH INSURANCE ADOPTION TREE</h1>
            <h2 class="subheading text--center text--white">ADOPT A RESIDENT OF IMO STATE TODAY!</h2>
            <div class="links flex">
              ...
              <template v-if="loggedInUser">
                <button class="button button--lg" @click="handleHealthPlanLink">Buy Health Plan</button>
              </template>
              <template v-else>
                <button
                  class="button button--lg button--grey"
                  @click="$store.commit('showGlobalForm', 'login')"
                >Buy Health Plan</button>
              </template>
              <a class="button button--lg text--no-decoration" href="#statistics">Statistics</a>
            </div>
          </section>
        </div>
      </div>
