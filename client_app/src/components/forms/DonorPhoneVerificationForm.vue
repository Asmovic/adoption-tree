<template>
  <div class="flex">
    <img src="/assets/img/side-registration-form-img.jpg" alt class="form__side-image" />
    <form action @submit.prevent="handleSubmit" class="global-form flex flex--column">
      <h1 class="text--center form-header text--bold mb-0">Phone Number Verification</h1>
      <h3
        class="text--center text--primary"
        style="line-height: 1.1"
      >Enter the one-time password sent To your mobile device</h3>
      <div class="activation-code mx-auto">
        ...
        <div class="row">
          <div class="col-sm">
            <button class="button col-sm" type="submit" :disabled="loading">{{ buttonText }}</button>
          </div>
        </div>
        <form-error />
      </div>
      <div class="confirmation-ui flex flex--column" v-if="verified" :class="{ animate: animate }">
      ...
      >
        <div class="msg">
          <h2 class="fw-md text--center">Do you want to adopt an Imolite now?</h2>
        </div>
    ...
    padding-right: 32px;"
          >Yes</button>
          <button class="button button--lg" @click="handleRouteChange" type="button">Remind Me Later</button>
        </div>
      </div>
    </form>
  </div>
